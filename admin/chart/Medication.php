...
$sql = "SELECT jenis_ubatubatan, COUNT(*) as count FROM elderly_resident GROUP BY jenis_ubatubatan";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $medicines[] = $row['jenis_ubatubatan'];
        $counts[] = $row['count'];
    }
} else {
    echo "No data found.";
    ...
    const medicationChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Distribution of Medications Among Residents',
