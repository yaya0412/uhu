...
$sql = "SELECT keadaan_semasa, COUNT(*) as count FROM elderly_resident GROUP BY keadaan_semasa"; // Updated to keadaan_semasa
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $currentConditions[] = $row['keadaan_semasa']; // Updated to fetch keadaan_semasa
        $counts[] = $row['count'];
    }
} else {
    echo "No data found.";
...
<head>
    ...
    <title>Current Conditions Distribution Pie Chart</title>
...
<h2>Current Conditions Distribution Pie Chart</h2>
...
<script>
    ...
    const conditionChart = new Chart(ctx, {
        type: 'pie',
        data: {
            labels: <?php echo json_encode($currentConditions); ?>, // Updated to use current conditions
            datasets: [{
                label: 'Number of Residents by Current Condition',
