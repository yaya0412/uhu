...
<?php
    ...
    if ($result_resident_count->num_rows > 0) {
        ...
        echo "<div class='result'> RESIDENT: <strong>" . $row_resident_count["jumlah_resident"] . "</strong></div><br>";
    } else {
        echo "<div class='result'>NO FOUND </div><br>";
    }

    // SQL untuk mendapatkan senarai semua resident_id dari table elderly_resident
    $sql_resident_list = "SELECT resident_id FROM elderly_resident";
    $result_resident_list = $conn->query($sql_resident_list);

    if ($result_resident_list->num_rows > 0) {
        echo "<div class='result'>Senarai resident_id:</div><ul class='list'>";
        while ($row_resident = $result_resident_list->fetch_assoc()) {
            echo "<li>" . $row_resident["resident_id"] . "</li>";
        }
        echo "</ul>";
    } 
    // Tutup sambungan
